...
namespace Shark_Remote.Engine.Bot
{
    public class AppValues
    {
        ...
        public static string[] botCommands { get; } = { "screen", "geo", "net", "ls", "ls0", "wh",
        ...
        "killps", "tasks", "cat", "msg", "start", "battery", "file", "dir", "del", "rd", "ping", "sc", "tprint",
         "info", "bot", "🗃 Файлы и папки", "🕹 Управление",
        ...
        public static string[] plugins()
        {
            try
            {
                return File.ReadAllLines($@"{Application.StartupPath}\data\plugins\installed.cfg");
            }
            catch (Exception ex)
            {
                return new string[] { ex.Message, ex.ToString() };
            }
        }
    }
    ...
    public class AppSettings
    {
        ...
        public static string Read(string section, string key, TomlTypeRead type = TomlTypeRead.OnlyOne, string sourcePath = @"settings\main.toml")
        {
            ...
            {
                string for_return = "";
                using (StreamReader reader = File.OpenText($@"{Application.StartupPath}\data\{sourcePath}"))
                {
        ...
        {
            string[] settings = File.ReadAllLines($@"{Application.StartupPath}\data\settings\variables.txt");
